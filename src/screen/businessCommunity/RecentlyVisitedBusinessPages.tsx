...
const RecentlyVisitedBusinessPages = ({navigation}: {navigation: any}) => {
  ...
  // function for get recently visited business data api call
  const getBusinessData = async () => {
    BusinessCommunityService.postAllRecentlyBusiness()
      .then((response: any) => {
        setPageLoader(false);
        setMyBusiness(response.data.recentlyvisited);
      })
      .catch(error => {
        setPageLoader(false);
        console.log(error);
  ...
  // function for search recently visited business data api call
  const getSearchBusinessData = (text: string) => {
    const data = {
      search: text,
    };
    setPageLoader(true);
    BusinessCommunityService.postSearchAllRecentlyBusiness(data)
      .then((response: any) => {
        if (response.data.recentlyvisited > 0) {
          setNoData(false);
          setMyBusiness(response.data.recentlyvisited);
          setPageLoader(false);
        } else {
  ...
  return (
    <View style={styles.container}>
      {/* header section */}
      <CustomHeader
        headerText={'Recently Visited Business Pages'}
        backButton={{
          visible: true,
          onClick: () => {
            navigation.goBack();
      ...
      {!pageLoader ? (
        myBusiness?.length > 0 ? (
          ...
          <View style={styles.noDataContainer}>
            {!noData ? (
              <Text style={styles.noDataText}>No Business Visited</Text>
            ) : (
...
export default RecentlyVisitedBusinessPages;
