...
const EditBusiness = ({navigation, route}: {navigation: any; route: any}) => {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      setYesNoText(route?.params?.data?.subscribed);
      setMonthYear(route?.params?.data?.subscriptonstatus);
      if (route?.params?.data?.price !== null) {
        setSubscriptionAmount(route?.params?.data?.price);
      }
      ...
      setBusinessSchema({
        ...
        businessAddress: route?.params?.data?.addressName,
  ...
  //  function for edit business submit button click
  const onSubmit = (values: any) => {
    ...
    if (yesNoText == '0') {
      ...
      feedBackData.append('businessid', route?.params?.data?.id);
      ...
      BusinessCommunityService.postEditBusiness(feedBackData)
        .then((response: any) => {
          setLoader(false);
        ...
        <Formik
          ...
          {({
            ...
            <View style={styles.body}>
              ...
              ) : route?.params?.data?.businessimage != null ? (
                <View style={styles.uploadMediaBox}>
                  <Image
                    style={styles.imageStyle}
                    resizeMode="cover"
                    source={{uri: `${route?.params?.data?.businessimage}`}}
                  />
                </View>
              ) : null}
