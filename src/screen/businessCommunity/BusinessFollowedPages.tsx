...
const BusinessFollowedPages = ({navigation}: {navigation: any}) => {
  ...
  // function for api call for get all my business data
  const getBusinessData = async () => {
    BusinessCommunityService.postMyAllFollowedBusiness()
      .then((response: any) => {
        setPageLoader(false);
        setMyBusiness(response.data.followedbusiness);
      })
      .catch(error => {
        setPageLoader(false);
        console.log(error);
  ...
  // function for api call for search business data
  const getSearchBusinessData = (text: string) => {
    const data = {
      search: text,
    };
    setPageLoader(true);
    BusinessCommunityService.postSearchMyAllFollowedBusiness(data)
      .then((response: any) => {
        if (response.data.followedbusiness > 0) {
          setNoData(false);
          setMyBusiness(response.data.followedbusiness);
          setPageLoader(false);
        } else {
      ...
      {!pageLoader ? (
        myBusiness?.length > 0 ? (
          ...
          <View style={styles.noDataContainer}>
            {!noData ? (
              <Text style={styles.noDataText}>No Business Followed. </Text>
            ) : (
...
export default BusinessFollowedPages;
