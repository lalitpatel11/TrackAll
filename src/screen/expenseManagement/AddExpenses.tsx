...
const AddExpense = ({navigation, route}: {navigation: any; route: any}) => {
  ...
  const handleCalendarSubmitClick = (selectDate: string) => {
    ...
    ExpensesManagementService.postCheckMonthData(data)
      .then((response: any) => {
  ...
  const handleCallApi = async (values: any) => {
    ...
    if (route?.params?.data?.budgetaryid != undefined) {
      feedBackData.append('budgetaryid', route?.params?.data?.budgetaryid);
    }
    if (route?.params?.data?.subcategoryid != undefined) {
